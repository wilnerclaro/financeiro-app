...
public interface CategoriaApiDoc {
    ...
    @Operation(summary = "Criar categoria", description = "Cria uma categoria financeira para um usuario.")
    ...
    @Operation(summary = "Listar categorias", description = "Lista categorias por usuario com filtros opcionais.")
    ...
    @Operation(summary = "Atualizar categoria", description = "Atualiza dados, tipo, categoria pai e status ativo.")
    ...
    @ApiResponse(responseCode = "404", description = "Categoria nao encontrada")
    ResponseEntity<CategoriaResponse> ativar(
            @PathVariable UUID id,
            @RequestParam UUID usuarioId
    );
}
